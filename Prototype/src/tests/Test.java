...
public class Test {
	...
	public Test(File testFile) {
		...
		try {
			...
			while (line != null && !line.equals("-- Expected Pattern --")) {
				InputCommands.add(line);

				line = reader.readLine();
			}

			if (line != null && line.equals("-- Expected Pattern --"))
				line = reader.readLine();
		...
		// It has to be in the end of the ctor, as we use the values set by ctor before
		// this.
		GetOutput();
	}
	...
	public boolean IsSuccessful() {
		ArrayList<String> outputCopy = (ArrayList)Output.clone();
		int outputLineIdx = 0;
		
		for (String line : ExpectedOutput) {
			boolean found = false;			
			
			for (int i = outputLineIdx; i<outputCopy.size(); i++) {
				String outputLine = outputCopy.get(i);
	...
	public static ArrayList<Test> GetAllTests() {
		...
		for (final File fileEntry : folder.listFiles()) {
			...
			String fileEntryType = fileEntryName.substring(fileEntryName.lastIndexOf('.') + 1, fileEntryName.length())
					.toLowerCase();
	...
	// Writes to src/tests/TestsFolder/Outputs/<FileName>_Output.txt
	private void WriteOutputToFile() {
		ArrayList<Test> tests = new ArrayList<Test>();
		File folder = new File("src/tests/TestsFolder/Outputs");
		
		
		try {
			BufferedWriter writer = new BufferedWriter(new FileWriter(folder + "/" + Name + "_Output.txt"));
