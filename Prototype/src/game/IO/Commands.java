 ...
 */
    ...
    public void Start() {GameManager.StartGame();}
    /*public void In(String path){
        ...
        if (f.exists()){
            ...
            while (true){
                try {
                    if ((line = br.readLine()) != null) command_interpreter.getInput(line);
   ...
   /* public void SpawnMechanic(String[] param){
    ...
    public void playerMove(String[] param){
        ...
        if (param.length < 2) System.err.println("Hibás bemenet!");
            ...
            if (neighbourIndex < 0 || neighbourIndex > p.GetCurrentPosition().GetNeighbours().size()) {System.err.println("Hibás mezőre lépés"); return;}
            else{
            ...
            if(s != null){
                if (s.GetType().equals("saboteur")) {
                    ...
                    switch (Integer.parseInt(sc.nextLine())) {
                        case 1:
                                    if (s.Damage()){
                                    GameManager.ActionExecuted();} break;
                        case 2:
                                    params = input.nextLine().split(" ");
                                    int neighbourIdxFrom = Integer.parseInt(params[0]);
                                    int neighbourIdxTo = Integer.parseInt(params[1]);
